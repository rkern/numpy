...
cdef class SFC64(BitGenerator):
    ...
    Chaotic PRNG ([1]_). Strictly speaking, SFC64 has a few different cycles
    that one might be on, depending on the seed, but its seeding should prevent
    most of the "short" ones. It also incorporates a 64-bit counter which means
    that the absolute minimum cycle length is :math:`2^{64}`. The expected
    period will be about :math:`2^{255}` ([2]_).
    ...
    **Compatibility Guarantee**
    ...
    ``SFC64`` makes a guarantee that a fixed seed and will always produce
