...
cdef class PCG32(BitGenerator):
    ...
    inc : {int}, optional
        Stream to return.
        Can be an integer in [0, 2**64], default is 0.Can be used with the same
        seed_seq to produce multiple streams using other values of inc.
    ...
    **Parallel Features**
    ...
    ``PCG32`` can also use sub-streams, which are generated by using the
    ...
    **Compatibility Guarantee**
    ...
    def __init__(self, seed_seq=None, inc=0):
        ...
        # Seed the _bitgen
        val = self._seed_seq.generate_state(1, np.uint64)
        ub = 2 * 64
